...
function resumableAppConfig(): unknown {
  const baseConfig = (webllm as any).prebuiltAppConfig;
  const modelList = (baseConfig?.model_list ?? []) as Array<
    Record<string, unknown>
  >;
  const modelRecord = modelList.find((record) => record.model_id === MODEL_ID);
  if (modelRecord === undefined) {
    throw new Error(`Cannot find model record for ${MODEL_ID}.`);
  }
  const integrity =
    modelRecord.integrity !== null && typeof modelRecord.integrity === "object"
      ? {
          ...(modelRecord.integrity as Record<string, unknown>),
          model_lib: undefined,
        }
...
function renderModelInfo(): void {
  ...
  modelLibEl.textContent = MODEL_LIB_URL;
}
